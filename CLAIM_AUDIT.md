...
| **Wireless Transport** | *"Physically validated Wi-Fi Direct P2P"* | **Native Wi-Fi Direct P2P + Local Wi-Fi & Bluetooth SPP**: Direct routerless Wi-Fi Direct P2P (`WifiDirectTransport.kt` via Android `WifiP2pManager`) was physically validated on physical dual handsets (Phone A: Galaxy A55 5G and Phone B: Note 10 Lite) on port 42889 with direct packet exchange and zero router dependency (Feature 28). Physical transport failover and DTN queue retention were validated under intentional disconnect/reconnect (Feature 29). | `WifiDirectTransport.kt`, `WifiTransport.kt`, `BluetoothTransport.kt`, `FEATURE28_WIFI_DIRECT_P2P_REPORT.md`, `FEATURE29_MULTI_HOP_FAILOVER_REPORT.md` |
...
### 3.5 Local Wi-Fi (Phone Hotspot) vs. Native Wi-Fi Direct (P2P Group Formation)
- **What iTantra Implements:** 
  1. `WifiTransport` binds to local UDP multicast/broadcast on port `42888` for ad-hoc LAN / phone-hotspot mesh.
  2. `WifiDirectTransport` (Feature 28) uses native Android `WifiP2pManager` to negotiate routerless P2P links directly between handsets on port `42889`. Physically validated on Phone A (`SM-A556E`) and Phone B (`SM-N770F`) without routers or hotspots.
  3. Feature 29 verifies transport failover, DTN store-and-forward retention during disconnects, and automatic recovery upon link restoration.
- **Boundaries and Qualifications:** Native Wi-Fi Direct operates as an autonomous single-hop transport link between paired nodes. Multi-hop relaying across $\ge 3$ nodes is executed at the user-space application layer (`ContextAwareRelayRouter`) and validated via synthetic automated test suites, not via hardware 802.11s kernel routing. Single-hop physical failover was tested across the 2-phone physical testbed.
